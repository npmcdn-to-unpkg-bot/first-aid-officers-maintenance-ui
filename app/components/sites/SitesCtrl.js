...
module.exports = function ($scope, $location, $cookies, dataSvc, busySvc, NgTableParams, ngDialog, clientSvc) {
  var init = true;
  var colsBase = [
    ...
    { title: 'D&eacute;partement', sortable: 'dept.dept_name', filter: { 'dept.dept_id': 'select' }, field: 'dept.dept_name', id: 'dept', show: true }
  ];
        ...
        callback: function (params, close) {
          ...
          Promise.all([clientSvc.getClientInfo(), clientSvc.getBase64Logo()]).then(_.spread(function (info, logo) {
            ...
            if (params.pdf) {
              ...
              report = sitesReports.generateXLSX(_.reject(_.filter($scope.cols, params.columns === 'all' ? _.stubTrue : 'show'), { id: 'button' }), data);
            }
