	...
	$scope.save = function() {
		...
		ngDialog.openConfirm({
			...
			var training = {
				...
				trainees: _.object(_.pluck($scope.trainees, 'empl_pk'), _.map($scope.trainees, function (trainee) {return _.defaults(trainee, {trem_outcome: 'SCHEDULED', trem_comment: ''});})) || _.object(_.pluck($scope.trainees, 'empl_pk'), _.map($scope.trainees, function () {return {trem_outcome: 'SCHEDULED'};}))
			};
